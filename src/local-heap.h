...
// LocalHeap 用来追踪所有访问gc堆的线程。LocalHeap 的状态要么是 Parked 要么是 
// Running。初始化时LocalHeap 为 Parked状态。
//  Parked: 禁止访问堆。
//  Running: 线程被允许访问堆但是必须让GC有机会手动调用Safepoint()。线程可以用
//           ParkedScope进入parked 状态.
...
class V8_EXPORT_PRIVATE LocalHeap {
 public:
  ...
  // 本地线程通过调用Safepoint方法检查主线程是否要求safepoint()
  void Safepoint() {
 ...
 private:
  ...
  // 指示local heap的当前状态，用来存储ThreadState
